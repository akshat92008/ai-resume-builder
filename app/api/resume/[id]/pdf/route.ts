...
function safeFilename(value: string) {
  return value.replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "resume";
}
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!IdSchema.safeParse(id).success) {
      return NextResponse.json({ error: { code: "INVALID_ID", message: "Invalid resume ID." } }, { status: 400 });
    }

    const rateLimit = await checkRateLimit(auth.user.id, getClientIp(request), "resume_pdf_export", 30);
    if (!rateLimit.allowed) {
      return NextResponse.json({ error: { code: "RATE_LIMIT_EXCEEDED", message: "PDF export limit reached. Try again later." } }, { status: 429 });
    }

    const resume = await getServerResume(id, auth.user.id);
    if (!resume) {
      return NextResponse.json({ error: { code: "RESUME_NOT_FOUND", message: "Resume not found." } }, { status: 404 });
    }

    const pdf = renderResumePdf(resume.content);
    const verification = await verifyResumePdfArtifact(resume.content, pdf);
    if (!verification.verified) {
      logger.error("[resume-pdf] Canonical PDF failed round-trip ATS verification", {
        resumeId: id,
        userId: auth.user.id,
        artifactScore: verification.artifactScore,
        missingSections: verification.missingSections,
        missingSignals: verification.missingSignals,
      });
      return NextResponse.json(
        {
          error: {
            code: "PDF_VERIFICATION_FAILED",
            message: "CareerOS could not verify this PDF artifact. Export was blocked rather than giving you an unverified file.",
    ...
    logger.error("[resume-pdf] Export failed", { error });
