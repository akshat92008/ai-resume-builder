...
export function resumeContentToPdfLines(content: CareerPathResumeContent): PdfLine[] {
  const lines: PdfLine[] = [];
  lines.push({ text: ascii(content.header.name) || "Resume", bold: true, size: 16 });
  const contact = [content.header.email, content.header.phone, content.header.location, content.header.links.linkedin, content.header.links.github, content.header.links.portfolio].map(ascii).filter(Boolean).join(" | ");
  ...
  section("Skills", content.skills.flatMap((group) => wrap(`${ascii(group.category)}: ${group.items.map(ascii).filter(Boolean).join(", ")}`).map((text) => ({ text }))), lines);

  const experienceLines: PdfLine[] = [];
  for (const item of content.experience) {
    wrap([item.role, item.company, item.dates, item.location].map(ascii).filter(Boolean).join(" | ")).forEach((text) => experienceLines.push({ text, bold: true }));
  ...
  for (const item of content.projects) {
    wrap([item.name, item.techStack.map(ascii).filter(Boolean).join(", "), item.link].map(ascii).filter(Boolean).join(" | ")).forEach((text) => projectLines.push({ text, bold: true }));
  ...
  for (const item of content.education) wrap([item.degree, item.institution, item.dates, item.score, item.location].map(ascii).filter(Boolean).join(" | ")).forEach((text) => educationLines.push({ text }));
  section("Education", educationLines, lines);
  section("Certifications", content.certifications.flatMap((item) => wrap([item.name, item.issuer, item.date, item.link].map(ascii).filter(Boolean).join(" | ")).map((text) => ({ text }))), lines);
...
function paginate(lines: PdfLine[]) {
  ...
  for (const line of lines) {
    ...
    const required = Math.max(LINE_HEIGHT, size + 3) + (line.gapBefore ?? 0);
    if (y - required < BOTTOM && pages.at(-1)!.length) { pages.push([]); y = TOP; }
...
export function renderResumePdf(content: CareerPathResumeContent): Buffer {
  ...
  pages.forEach((pageLines, index) => {
    ...
    objects.set(contentObject, Buffer.concat([Buffer.from(`<< /Length ${stream.length} >>\nstream\n`, "ascii"), stream, Buffer.from("endstream", "ascii")]));
  ...
  chunks.push(Buffer.from([...xref, "trailer", `<< /Size ${maxObject + 1} /Root 1 0 R >>`, "startxref", String(xrefOffset), "%%EOF", ""].join("\n"), "ascii"));
