...
export function verifyExtractedResumeText(content: CareerPathResumeContent, extractedText: string): ATSArtifactVerification {
  ...
  const expectedLines = resumeContentToPdfLines(content)
  ...
  return {
    artifactScore,
    textCoverage: Math.round(textCoverage * 1000) / 10,
    sectionCoverage: Math.round(sectionCoverage * 1000) / 10,
    verified: artifactScore >= 95 && missingSections.length === 0 && missingSignals.length === 0,
    missingSections,
    missingSignals,
    warnings,
...
export async function extractPdfText(buffer: Buffer): Promise<string> {
  ...
  // pdfjs-dist expects browser geometry globals even when only extracting text.
...
export async function verifyResumePdfArtifact(content: CareerPathResumeContent, buffer: Buffer) {
  ...
  return verifyExtractedResumeText(content, text);
}
