...
export function runCareerProofAgent(input: CareerProofAgentInput): CareerProofAgentResult {
  ...
  const canGenerate = vaultReport.canGenerateResume || input.forceResumeGeneration;
  const resume = shouldGenerateResume && canGenerate
  ...
  if (shouldGenerateResume && !canGenerate) {
    warnings.push("Your resume will be weak if I generate it now. Improve the missing project and skill details first, or generate a draft anyway.");
  }
