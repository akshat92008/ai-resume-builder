...
import { 
  getAdminOrdersServer, getAdminLeadsServer, getAdminUsersServer, 
  getAdminMetricsServer, getAdminEventsServer, getAdminTestimonialsServer, 
  saveAdminTestimonialServer, deleteAdminTestimonialServer 
} from "./admin-actions";
...
export async function rejectAdminOrder(orderId: string, reason?: string): Promise<boolean> {
  const response = await fetch("/api/admin/orders/reject", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order_id: orderId, reason }),
  });
  return response.ok;
}
...
export async function updateAdminLeadStatus(leadId: string, status: NonNullable<Lead["status"]>): Promise<Lead | null> {
  const response = await fetch("/api/admin/leads/update", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lead_id: leadId, status }),
  });
...
export async function updateAdminUserPlan(userId: string, plan: Profile["plan"] = "free"): Promise<boolean> {
  const response = await fetch("/api/admin/users/update-plan", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, plan }),
  });
  return response.ok;
}
