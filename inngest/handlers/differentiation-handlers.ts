...
export async function handleEstimateImpact(currentResume: CareerPathResume | null, userId: string, metadata: { userId: string; resumeId?: string }) {
  if (!currentResume) return { assistantMessage: "Build a resume first, then I can analyze your bullets and identify where a verifiable metric would strengthen the evidence.", resume: null, resumeId: null, workspace: buildCareerWorkspaceState(null) };
  const profile = currentResume.careerProfile || legacyProfileToCareerProfile(currentResume.profile, userId);
  ...
  if (!result.suggestions.length) return { assistantMessage: "Your bullets already have good quantitative proof. No weak metrics detected that need investigation.", resume: currentResume, resumeId: currentResume.id, workspace: buildCareerWorkspaceState(currentResume) };
  const suggestionList = result.suggestions.slice(0, 4).map((s, i) => `${i + 1}. **${s.itemName}** (${s.section})\n   Original: _"${s.bulletText.slice(0, 80)}..."_\n   Candidate wording: "${s.improvedBullet}"\n   Confidence: ${s.confidence} — ${s.rationale}`).join("\n\n");
  return { assistantMessage: `Found ${result.suggestions.length} bullet${result.suggestions.length !== 1 ? "s" : ""} where a metric may improve clarity. Treat these as questions to verify, not facts to paste blindly:\n\n${suggestionList}\n\nOpen **More** to review each impact suggestion.`, resume: currentResume, resumeId: currentResume.id, workspace: buildCareerWorkspaceState(currentResume) };
}
...
export async function handleGapAnalysis(message: string, currentResume: CareerPathResume | null, userId: string, metadata: { userId: string; resumeId?: string }) {
  ...
  return { assistantMessage: `**Gap Analysis for ${targetRole}**\n\nMatch score: **${result.matchScore}/100** — ${status}\n\n${result.summary}\n\n**Gaps to address:**\n${gapList}\n\nOpen **More** for the full gap breakdown and project ideas.`, resume: currentResume, resumeId: currentResume.id, workspace: buildCareerWorkspaceState(currentResume) };
}
...
export async function handleMultiPersona(currentResume: CareerPathResume | null, userId: string, metadata: { userId: string; resumeId?: string }) {
  if (!currentResume) return { assistantMessage: "Build a master resume first, then I can generate distinctly positioned versions targeting different roles.", resume: null, resumeId: null, workspace: buildCareerWorkspaceState(null) };
  const profile = currentResume.careerProfile || legacyProfileToCareerProfile(currentResume.profile, userId);
...
export async function handleVisualizeATS(currentResume: CareerPathResume | null, _userId: string, metadata: { userId: string; resumeId?: string }) {
  if (!currentResume) return { assistantMessage: "Build a resume first, then say 'show ATS view' to verify the canonical PDF CareerOS would export.", resume: null, resumeId: null, workspace: buildCareerWorkspaceState(null) };

  const contentReview = await generateATSViewAgent(currentResume.content, currentResume.targetRole, metadata);
  const canonicalPdf = renderResumePdf(currentResume.content);
  const artifact = await verifyResumePdfArtifact(currentResume.content, canonicalPdf);

  const artifactSection = {
    sectionName: "Canonical PDF round-trip",
    rawText: `Text coverage ${artifact.textCoverage}%; section coverage ${artifact.sectionCoverage}%; extracted characters ${artifact.extractedCharacters}.`,
    issues: artifact.warnings.map((warning) => ({
      type: "parse_failure" as const,
      description: warning,
      severity: artifact.verified ? "low" as const : "high" as const,
    })),
    atsScore: artifact.artifactScore,
  };

  const result = {
    ...contentReview,
    sections: [...contentReview.sections, artifactSection],
    overallATSScore: Math.min(contentReview.overallATSScore, artifact.artifactScore),
    criticalFailures: artifact.verified
      ? contentReview.criticalFailures
      : [...contentReview.criticalFailures, ...artifact.warnings],
    passedChecks: artifact.verified
      ? [...contentReview.passedChecks, `Canonical exported PDF round-tripped at ${artifact.artifactScore}/100 with all key sections/evidence recovered.`]
      : contentReview.passedChecks,
    summary: `${contentReview.summary} Canonical PDF verification: ${artifact.verified ? "passed" : "failed"} (${artifact.artifactScore}/100 artifact score).`,
  };
  ...
  return { assistantMessage: `**ATS Compatibility: ${result.overallATSScore}/100** ${statusEmoji}\n\n${result.summary}\n\n**Artifact verification:** ${artifact.verified ? `passed — ${artifact.textCoverage}% text coverage after parsing the exact canonical PDF` : `failed — ${artifact.warnings.join("; ")}`}\n\n${result.criticalFailures.length > 0 ? `**${result.criticalFailures.length} critical issue${result.criticalFailures.length !== 1 ? "s" : ""} detected:** ${result.criticalFailures.join("; ")}` : "**No critical ATS failures detected.**"}\n\nOpen **More** for the section-by-section content review plus PDF round-trip check.`, resume: currentResume, resumeId: currentResume.id, workspace: buildCareerWorkspaceState(currentResume) };
}
...
export async function handleGenerateOutreach(message: string, currentResume: CareerPathResume | null, userId: string, metadata: { userId: string; resumeId?: string }) {
  if (!currentResume) return { assistantMessage: "Build a resume first, then paste a job description and say 'write cover letter' or 'write outreach' to generate your application messaging.", resume: null, resumeId: null, workspace: buildCareerWorkspaceState(null) };
  const profile = currentResume.careerProfile || legacyProfileToCareerProfile(currentResume.profile, userId);
