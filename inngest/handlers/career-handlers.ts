...
export async function handleGenerateApplicationPack(
  message: string,
  ...
  if (!resume) {
    return {
      assistantMessage:
        "I need a resume or enough career details before I can prepare the full application pack. Paste your career info and the job description together.",
      resume: null,
      resumeId: null,
  ...
  decorateResumeForCareerOS(resume, message, {
    versionType: "job_specific",
  });
  const pack = generateApplicationPack(
    resume.careerProfile!,
    resume,
    job,
  );
  resume.applicationPack = pack;
  resume.jobSearchInsights = analyzeJobSearchPerformance(
    resume.applications || [],
...
export async function handleTrackJobApplication(
  message: string,
  currentResume: CareerPathResume | null,
  userId: string,
) {
  if (!currentResume) {
    return {
      assistantMessage:
        "I can track applications after there is a resume in the workspace. Build or paste your resume details first, then say what job you applied to.",
      resume: null,
      resumeId: null,
  ...
  const job = currentResume.jobDescription
    ? extractJobDescription(currentResume.jobDescription)
    : extractJobDescription(message);
  const application = createJobApplicationFromCommand(
    message,
    userId,
    currentResume,
    job,
  ...
  const applications = [
    application,
    ...(currentResume.applications || []),
  ].slice(0, MAX_TRACKED_APPLICATIONS);
  currentResume.applications = applications;
  currentResume.jobSearchInsights = analyzeJobSearchPerformance(
    applications,
...
export async function handleAnalyzeJobSearch(
  currentResume: CareerPathResume | null,
) {
  if (!currentResume) {
    return {
      assistantMessage:
        "I need tracked applications before I can analyze your job search. Start by tracking saved or applied jobs.",
      resume: null,
      resumeId: null,
  ...
  const insights = analyzeJobSearchPerformance(
    currentResume.applications || [],
    [currentResume.resumeDocument!],
  );
