...
export const processResumeIntent = (inngest as any).createFunction(
  ...
  { event: "resume/process.intent" },
  async ({ event, step }: { event: any, step: any }) => {
    const { intent, message, currentResume, userId, resumeId, command } = event.data;
    
    const result = await step.run("process-intent", async () => {
      return processIntent(intent, message, currentResume, userId, resumeId, command);
    });
